        ...
        <div class="card-items flex justify-left items-center">
          ...
          <div class="ml-2 w-4/5">
            <div class="flex justify-between items-end">
              <h1 class="text-sm font-medium sm:text-xl md:text-2xl lg:text-3xl xl:text-4xl">
                {{ item.item_name }}
              </h1>
            </div>
            <ProgressBar
            ...
            <div class="block sm:flex sm:justify-between sm:items-center text-xs sm:text-lg md:text-base">
              <h1 class="text-[0.6rem] sm:text-sm md:text-base lg:text-lg xl:text-xl">
                <span class="pump-animation">RM{{ item.max_amount - item.current_amount }}.00 </span>
                left
              </h1>
              <router-link
                :to="getContributionLink(item.id)"
                class="my-1 w-full sm:w-3/5 bg-green-200 sm:p-1 rounded-sm focus:text-white text-xs font-bold sm:text-lg md:text-xl lg:text-2xl xl:text-3xl rounded-md text-center"
              >
                <button>
                  Contribute
                </button>
...
// Method to construct the router link
const getContributionLink = (index) => {
  console.log("item.id: " + index)
  return {
    path: "./paymentSponsorNecessary",
    query: { id: index, table_name: "sponsor_records" },
  };
};


//Use onMounted to display the latest value of the total amount being funded for each item
